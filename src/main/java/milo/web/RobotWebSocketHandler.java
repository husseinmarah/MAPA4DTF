...
public class RobotWebSocketHandler extends TextWebSocketHandler {
    ...
    private List<ConveyorDTO> getConveyors() throws ExecutionException, InterruptedException {
        ...
        for (int i = 0; i < numConveyors; i++) {
            ...
            boolean produced = readBooleanValue(CustomNamespace.inputConveyors.get(i).getProducedNode().getNodeId()
                    .getIdentifier().toString());
            boolean enabled = readBooleanValue(CustomNamespace.inputConveyors.get(i).getEnabledNode().getNodeId()
                    .getIdentifier().toString());
